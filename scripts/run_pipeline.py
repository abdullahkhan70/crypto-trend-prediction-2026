...
def main(csv: str = None):
    ...
    mae, rmse, model = train_random_forest(X_train_s, y_train.values, X_test_s, y_test.values, save_dir=save_dir)
    print(f'RF MAE: {mae:.4f}, RMSE: {rmse:.4f}')

    # Optional: XGBoost (if installed)
    try:
        print('Attempting to train XGBoost (if available)...')
        mae_xgb, rmse_xgb, xgb_model = train_xgboost(X_train_s, y_train.values, X_test_s, y_test.values, save_dir=save_dir)
    ...
    try:
        ...
        mae_lstm, rmse_lstm, lstm_model = train_lstm_sequence(arr, seq_len=14, save_dir=save_dir)
